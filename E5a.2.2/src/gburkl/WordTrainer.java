...
public class WordTrainer implements Serializable {
    private static final long serialVersionUID = -3839687616431658736L;
    ...
    /**
     * Adds a new {@link WordEntry} to the associated {@link WordList}
     * @param word the word for the new {@link WordEntry}
     * @param url the url for the new {@link WordEntry}
     */
    public void addWord(String word, String url) {
        this.list.add(new WordEntry(word, url));
    }

    /**
     * Sets all values from the read object
     * @param readObject the object read from file
     */
    public void setValuesFrom(WordTrainer readObject) {
        this.list = readObject.list;
        this.correct = readObject.correct;
        this.checked = readObject.checked;
        this.current = readObject.current;
    }
}
